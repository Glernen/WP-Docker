...
class new_cat extends WP_Widget {
		...
		<div class="list-grid list-rounded my-n2">
			...
			<div class="list-item py-2">
				...
				<div class="media media-3x2 rounded col-4 mr-3">
                    ...
                    <a class="media-content" href="<?php the_permalink(); ?>" <?php echo $newWindow ?> title="<?php the_title(); ?>" data-src="<?php echo io_get_thumbnail(array('width'=>200,'height'=>150),true) ?>"></a>
                    <?php else: ?>
                    <a class="media-content" href="<?php the_permalink(); ?>" <?php echo $newWindow ?> title="<?php the_title(); ?>" style="background-image: url(<?php echo io_get_thumbnail(array('width'=>200,'height'=>150),true) ?>);"></a>
                    <?php endif ?>
				</div>
				<?php } ?>
	...
	function form( $instance ) {
		...
		$instance = wp_parse_args( (array) $instance, array( 
			...
			<p>
				<label for="<?php echo $this->get_field_id('title_ico'); ?>">图标标题：</label>
...
class new_bulletin extends WP_Widget {
	...
	<div class="card-body"> 
		...
		<div class="mt-4"><a href="<?php echo get_permalink(io_get_option('all_bull')) ?>" target="_blank" class="btn btn-outline-danger btn-block">更多</a></div>
	</div>
...
class about extends WP_Widget {
    ...
    <div class="widget-author-meta text-center p-4">
	    ...
	    <div class="row no-gutters text-center my-3">
			...
			<?php if($instance['tqqurl']) { ?> 
	      	<div class="col">
			  <a target=blank rel="external nofollow" href=http://wpa.qq.com/msgrd?V=3&uin=<?php echo $instance['tqqurl']; ?>&Site=QQ&Menu=yes  data-toggle="tooltip" data-placement="top" title="QQ在线"><i class="iconfont icon-qq icon-lg"></i></a>
	      	</div>
			<?php } ?>
			<?php if($instance['tsinaurl']) { ?> 
	      	<div class="col">
			  <a href="<?php echo $instance['tsinaurl']; ?>" target="_blank"  data-toggle="tooltip" data-placement="top" title="微博" rel="external nofollow"><i class="iconfont icon-weibo icon-lg"></i></a>
			</div>
			<?php } ?>
	    ...
	    <div class="row no-gutters text-center">
	      	<div class="col">
	        	<span class="font-theme font-weight-bold text-md"><?php echo wp_count_posts('sites')->publish ?></span><small class="d-block text-xs text-muted">收录网站</small>
	      	</div>
	      	<div class="col">
	        	<span class="font-theme font-weight-bold text-md"><?php $count_posts = wp_count_posts(); echo $published_posts = $count_posts->publish;?></span><small class="d-block text-xs text-muted">文章</small>
	      	</div>
	      	<div class="col">
	        	<span class="font-theme font-weight-bold text-md"><?php author_posts_views(); ?></span><small class="d-block text-xs text-muted">访客</small>
	      	</div>
	    </div>
	</div>
	<?php
	...
	function form($instance) {
		...
		$instance = wp_parse_args( (array) $instance, $defaults ); 
		global $wpdb;
		$instance = wp_parse_args((array) $instance, array('weixin' => '' . get_template_directory_uri() . '/images/wechat_qrcode.png"'));
		$weixin = $instance['weixin'];
		$instance = wp_parse_args((array) $instance, array('about_img' => '' . get_template_directory_uri() . '/images/avatar.png"'));
	...
	<p>
		<label for="<?php echo $this->get_field_id('tsinaurl'); ?>">新浪微博地址：</label>
	...
	<p>
		<label for="<?php echo $this->get_field_id('github'); ?>">GitHub地址：</label>
...
class about_author extends WP_Widget {
    ...
    <div class="widget-author-meta text-center p-4">
	    ...
	    <div class="row no-gutters text-center">
	      	...
	      	<a href="<?php echo $author_url ?>" target="_blank" class="col">
	        	<span class="font-theme font-weight-bold text-md"><?php echo $comment_count;?></span><small class="d-block text-xs text-muted">评论</small>
	      	</a>
	      	<a href="<?php echo $author_url ?>" target="_blank" class="col">
	        	<span class="font-theme font-weight-bold text-md"><?php author_posts_views(get_the_author_meta('ID'));?></span><small class="d-block text-xs text-muted">浏览</small>
	      	</a>
	      	<a href="<?php echo $author_url ?>" target="_blank" class="col">
	        	<span class="font-theme font-weight-bold text-md"><?php author_posts_likes(get_the_author_meta('ID'));?></span><small class="d-block text-xs text-muted">获赞</small>
	      	</a>
	    </div>
	</div>
...
class hot_post_img extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			...
			'description' => __( '调用点击最多的文章，需开启主题设置里的“访问统计”,并有统计数据' ),
			'customize_selective_refresh' => true,
		);
...
class hot_sites extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname' => 'hot_sites',
			'description' => __( '调用点击最多的网址，需开启主题设置里的“访问统计”,并有统计数据' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct('hot_sites', '热门网址', $widget_ops);
	}
